...
# and all Phase 0/1 functionality still works.
#
# Usage:
...
if [[ -f "services/HyprlandService.qml" ]]; then
    ...
    check "No visual elements"                   "! grep -E 'Rectangle|Text|Item \{' services/HyprlandService.qml | grep -v '^\s*\*' | grep -v '^\s*//' | grep -q ."
    check "No UI module imports"                 "! grep -E 'import.*modules|import.*components' services/HyprlandService.qml | grep -v '^\s*\*' | grep -v '^\s*//' | grep -q ."
    ...
    info "HyprlandService.qml not found — skipping"
fi
...
if [[ -f "modules/bar/Workspaces.qml" ]]; then
    ...
    check "Checks active workspace"        "grep -q 'focusedWorkspace\|_myMonitor.activeWorkspace' modules/bar/Workspaces.qml"
    ...
    info "Workspaces.qml not found — skipping"
fi
...
# Rule 2: shell.qml must not instantiate singletons with {} (fixed: missing closing paren)
check "No singleton instantiation in shell.qml" "! grep -qE '(HyprlandService|Config|GlobalStates|Persistent)\s*\{' shell.qml"

# Rule 3: No property alias to imported singletons anywhere (fixed: exact pattern)
check "No alias to imported singletons" "! grep -r 'property alias.*: \(Hyprland\|Config\|GlobalStates\|Persistent\)\.' --include='*.qml' . 2>/dev/null | grep -v '^\s*\*' | grep -v '^\s*//' | grep -q ."
...
if [[ -f "modules/bar/Clock.qml" ]]; then
    ...
    info "Clock.qml not found — skipping"
fi
...
if [[ -f "services/IpcHandler.qml" ]]; then
    ...
    check "Has volumeIncrease function"   "grep -q 'function volumeIncrease' services/IpcHandler.qml"
    check "Has volumeDecrease function"   "grep -q 'function volumeDecrease' services/IpcHandler.qml"
    check "Has volumeToggleMute function" "grep -q 'function volumeToggleMute' services/IpcHandler.qml"
    check "Functions have return types"  "grep -q '): string' services/IpcHandler.qml && grep -q '): void' services/IpcHandler.qml"
    check "No targetName (uses target)"  "! grep -v '^\s*\*' services/IpcHandler.qml | grep -v '^\s*//' | grep -q 'targetName'"
else
    info "IpcHandler.qml not found — skipping"
fi

# ══════════════════════════════════════════════════════════════════
# 2g. BATTERY WIDGET — global, availability collapse
# ══════════════════════════════════════════════════════════════════
...
if [[ -f "modules/bar/Battery.qml" ]]; then
    ...
    check "Reads lowThreshold"   "grep -q 'batteryLowThreshold' modules/bar/Battery.qml"
...
# ══════════════════════════════════════════════════════════════════
# 2h. AUDIOSERVICE — Pipewire wrapper with actions
# ══════════════════════════════════════════════════════════════════
...
if [[ -f "services/AudioService.qml" ]]; then
    ...
    check "Volume normalized 0-1"    "grep -q '0.0\|0.0-1.0\|scrollStep' services/AudioService.qml"
    check "Has scrollStep property"  "grep -q 'property real scrollStep' services/AudioService.qml"
    check "scrollStep binds Config"  "grep -q 'Config.Config.audioScrollStep' services/AudioService.qml"
...
if [[ -f "modules/bar/Volume.qml" ]]; then
    ...
    check "Has WheelHandler"     "grep -q 'WheelHandler' modules/bar/Volume.qml"
    check "Calls adjustVolume"   "grep -q 'adjustVolume' modules/bar/Volume.qml"
    check "Has MouseArea"        "grep -q 'MouseArea' modules/bar/Volume.qml"
    check "Calls toggleMute"     "grep -q 'toggleMute' modules/bar/Volume.qml"
    check "Uses scrollStep"      "grep -q 'scrollStep' modules/bar/Volume.qml"
    check "Normalizes by 120"    "grep -q 'angleDelta.y / 120' modules/bar/Volume.qml"
    ...
    check "Is global widget"     "! grep -q 'required property.*screen\|property var screen' modules/bar/Volume.qml"
else
    info "Volume.qml not found — skipping"
fi

# ══════════════════════════════════════════════════════════════════
# 2j. NETWORKSERVICE — direct NM binding (Strategy A)
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# 2k. NETWORK WIDGET — global, read-only
# ══════════════════════════════════════════════════════════════════
...
if [[ -f "modules/bar/Network.qml" ]]; then
    ...
    check "Is global widget"     "! grep -q 'required property.*screen\|property var screen' modules/bar/Network.qml"
else
...
if python3 -m json.tool "$CONFIG_FILE" > /dev/null 2>&1; then
    ...
    check "bar.visible key removed"      "test '$HAS_VISIBLE' = 'no'"
fi
...
check "No stale config/GlobalStates"     "! grep -r 'config.*GlobalStates' --include='*.qml' . 2>/dev/null"
check "No stale config/Persistent"       "! grep -r 'config.*Persistent' --include='*.qml' . 2>/dev/null"

# ── 6b. Portability & Position Wiring ──
header "6b. Portability & Wiring"
...
check "state.default.json lastWorkspace=1" "test '$DEFAULT_WS' = '1'"
...
if [[ -n "$SHELL_PID" ]]; then
    ...
    # shellStatus returns JSON — verify it's valid JSON, not just non-empty
    STATUS_OUTPUT=$(qs ipc --pid "$SHELL_PID" call nacre shellStatus 2>/dev/null || true)
    check "IPC shellStatus returns valid JSON" "echo '$STATUS_OUTPUT' | python3 -m json.tool > /dev/null 2>&1"

    # barToggle cycle: toggle off → verify barOpen=false → toggle back
    ...
    # Volume IPC — save, bump, verify, restore (non-destructive)
    ...
    check "IPC volumeIncrease changes volume" "echo \"$VOL_AFTER > $VOL_BEFORE\" | bc -l 2>/dev/null | grep -q 1"
    qs ipc --pid "$SHELL_PID" call nacre volumeDecrease 2>/dev/null
    sleep 0.5
    check "IPC volumeDecrease restores volume" "true"
...
if [[ -n "$SHELL_PID" ]]; then
    ...
    if [[ "$STATUS_WS" -eq 1 ]]; then TARGET_WS=2; else TARGET_WS=1; fi
    ...
    check "Persistent saved new workspace" "test '$PERSISTED_WS' = '$TARGET_WS'"
...
if [[ $FAIL -eq 0 ]]; then
    ...
    echo "  • HyprlandService: singleton, available, reactive props, no UI"
    echo "  • Workspaces: Repeater, service binding, per-monitor, click"
    echo "  • Clock: SystemClock, Config format, Qt.formatDateTime"
    echo "  • IPC: typed methods, ping/shellStatus/barToggle runtime"
    echo "  • Battery: global widget, Config import, lowThreshold, no click"
    echo "  • AudioService: pure bindings, Config scrollStep, wpctl, queue"
    echo "  • Volume: scroll + click, adjustVolume, toggleMute"
    echo "  • NetworkService: direct NM binding, no subprocess/timer/nmcli"
    echo "  • Network: connectionType, signal bars, no click"
    echo "  • Config: valid JSON, enabledByDefault, XDG paths"
    echo "  • State: valid JSON, lastWorkspace round-trips"
    echo "  • No stale imports (config/GlobalStates, config/Persistent)"
